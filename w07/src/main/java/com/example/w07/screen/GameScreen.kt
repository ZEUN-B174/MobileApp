...
fun GameScreen(
    ...
    Column(
        ...
        // 사용자 및 점수 정보
        Text(
            // text = "플레이어: ${uiState.userData.username} (총점: ${uiState.userData.totalScore}점)",
            text = "플레이어: taewony (총점: 0점)",
            fontSize = 18.sp,
            modifier = Modifier.padding(bottom = 8.dp)
        )
        Text(
            text = "현재 레벨: 0", // ${uiState.userData.level}",
            fontSize = 18.sp,
        ...
        // 설정 정보
        Text(
            text = "목표 시간: ${formatTime(300)}", //uiState.config.targetTimeMs)}",
            style = MaterialTheme.typography.titleLarge
        )
        Text(
            text = "오차 범위: ±${formatTime(20)}", //uiState.config.toleranceMs)}",
